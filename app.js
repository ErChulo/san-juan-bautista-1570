...
function islandPage(slug){
  ...
  const rescueBlock=rescue ? `<section class="section section-tight"><div class="rescue-card"><div class="rescue-meta"><span>LO QUE EL VIDEO ILUSTRA COMO «RESCATES»</span><strong>${esc(rescue.date)}</strong></div><h2>${esc(rescue.title)}</h2><p>${esc(rescue.text)}</p><a href="${rescue.sourceUrl}" target="_blank" rel="noreferrer">${esc(rescue.sourceTitle)} ↗</a></div></section>` : '';
  return shell(`
